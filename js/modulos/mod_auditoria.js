...
window.ModAuditoria = {
    ...
    aplicarFiltros: function() {
        ...
        let filtroEscuela = document.getElementById('filtro-escuela-auditoria') ? document.getElementById('filtro-escuela-auditoria').value : "TODAS";
    ...
    renderizarTabla: function() {
        ...
        if (datosPagina.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center py-5 text-muted"><i class="bi bi-clipboard-x fs-1 d-block mb-3"></i>No hay registros para este filtro.</td></tr>`;
        ...
        datosPagina.forEach(r => {
            ...
            let badgeEscuela = r.escuela === 'lb' ? `<span class="badge bg-primary text-white">L.B.</span>` : (r.escuela === 'sb' ? `<span class="badge bg-success text-white">S.B.</span>` : `<span class="badge bg-dark">N/A</span>`);
    ...
    exportarCSV: function() {
        ...
        if (datos.length === 0) return Swal.fire('Aviso', 'No hay datos para exportar con el filtro actual.', 'warning');
    ...
    mantenimientoTotal: function() {
        ...
        let filtroEscuela = document.getElementById('filtro-escuela-auditoria') ? document.getElementById('filtro-escuela-auditoria').value : "TODAS";
        let datosAfectados = this.registrosFiltrados; // Se basa en el filtro actual

        if (datosAfectados.length === 0) {
            return Swal.fire('Aviso', 'No hay registros para la escuela seleccionada.', 'warning');
        }

        let tituloMensaje = filtroEscuela === "TODAS" ? "Mantenimiento Total (Ambas Escuelas)" : `Mantenimiento: ${filtroEscuela === 'lb' ? 'Libertador Bolívar' : 'Santa Bárbara'}`;
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // 1. Exportamos los registros filtrados a Excel
                if(typeof XLSX === 'undefined') return Swal.fire('Error', 'La librería Excel no está cargada.', 'error');
                
                let dataLimpia = datosAfectados.map(r => ({
                    "Fecha y Hora": new Date(r.fecha).toLocaleString('es-VE'),
                    "Escuela": r.escuela === 'lb' ? 'Libertador Bolívar' : (r.escuela === 'sb' ? 'Santa Bárbara' : r.escuela),
                    "Usuario": r.usuario_nombre,
                    "Cédula": r.usuario_cedula,
                ...
                // 2. Vaciamos los registros de Supabase correspondientes al filtro
                window.Aplicacion.mostrarCarga();
                try {
                    let query = window.supabaseDB.from('historial_auditoria').delete().not('id', 'is', null);
                    ...
                    Swal.fire('Servicio Completado', 'El respaldo se descargó y el historial fue vaciado exitosamente para la selección actual.', 'success');
                    
                    // 3. Auditoría de la propia limpieza (Queda como el nuevo primer evento)
                    let codEscuelaAuditoria = filtroEscuela !== "TODAS" ? filtroEscuela : 'Ambas';
                    window.Aplicacion.auditar('Auditoría del Sistema', 'Mantenimiento', `Se descargó respaldo y se vació historial de ${codEscuelaAuditoria} (${datosAfectados.length} eventos).`);
