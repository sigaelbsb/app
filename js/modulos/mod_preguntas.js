    ...
    eliminarPregunta: function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.Aplicacion.mostrarCarga();
                try {
                    const { data, error } = await window.supabaseDB.from('conf_preguntas_seguridad').delete().eq('id', id).select();
                    window.Aplicacion.ocultarCarga();
                    if(error) throw error;
                    
                    if(data && data.length === 0) {
                        return Swal.fire('Atención', 'No se pudo eliminar la pregunta. Verifique si tiene permisos o si la pregunta ya fue eliminada.', 'warning');
                    }
                    
                    Swal.fire('¡Éxito!', 'La pregunta ha sido eliminada de la base de datos.', 'success');
