...
const SUPABASE_URL = 'https://nbsrlauuugxfcgjavfve.supabase.co';
const SUPABASE_KEY = 'sb_publishable_5fWhLgihhav9Vu-t2HdyYg_pnayrzg7';
...
window.Aplicacion = {
    ...
    init: async function() { 
        ...
        if (guardado) { 
            ...
            setTimeout(() => { 
                ...
                if (localStorage.getItem('sigae_tiene_huella')) {
                    document.getElementById('btn-biometrico-container').style.display = 'block';
                }
            }, 1000); 
        } 
    },
        ...
        try {
            ...
            if(rolesRes.data) {
                ...
                if (miRol && miRol.permisos) {
                    try {
                        let todosPermisos = typeof miRol.permisos === 'string' ? JSON.parse(miRol.permisos) : miRol.permisos;
                        let codigoEscuela = (this.usuario && this.usuario.id_escuela) ? this.usuario.id_escuela : (localStorage.getItem('sigae_escuela_codigo') || 'sb');
                        
                        // Si el JSON tiene la estructura anidada nueva, extraemos la escuela activa
                        if (todosPermisos.lb || todosPermisos.sb) {
                            this.permisosActuales = todosPermisos[codigoEscuela] || {};
                        } else {
                            // Retrocompatibilidad temporal si hay permisos viejos
                            this.permisosActuales = todosPermisos || {};
                        }
                    } catch(e) {
            ...
            if(this.usuario) { 
                ...
                const codigoEscuela = this.usuario.id_escuela || localStorage.getItem('sigae_escuela_codigo') || 'sb';
                const nombreEscuela = this.usuario.nombre_escuela || localStorage.getItem('sigae_escuela_activa') || 'UE Santa Bárbara';
                
                document.querySelectorAll('.logo-img').forEach(img => {
                    img.src = 'assets/img/logo_' + codigoEscuela + '.png';
                    img.onerror = function() { this.src = 'assets/img/sigae.png'; };
                });
                
                const textoSidebar = document.querySelector('.sidebar-texto');
                if(textoSidebar) textoSidebar.innerText = 'SIGAE ' + nombreEscuela.replace('UE ', '');
            }
    ...
    tieneAccesoEscuela: function(codigoEscuela) {
        if(this.usuario && this.usuario.rol === 'SuperAdmin') return true;
        if(!this.rolesDelSistema) return false;
        
        let miRol = this.rolesDelSistema.find(r => r.nombre === this.usuario.rol);
        if(!miRol || !miRol.permisos) return false;
        
        try {
            let todosPermisos = typeof miRol.permisos === 'string' ? JSON.parse(miRol.permisos) : miRol.permisos;
            let privsEscuela = todosPermisos[codigoEscuela];
            if(!privsEscuela) return false;
            // Verificamos si existe el acceso explícito al plantel guardado
            if(privsEscuela.hasOwnProperty('__acceso_plantel__')) {
                return privsEscuela['__acceso_plantel__'].ver === true;
            }
            
            // Retrocompatibilidad: Si no se guardó con el switch explícito aún, ver si tiene ALGO asignado
            let tieneOtrosAccesos = false;
            for(let mod in privsEscuela) {
                if(privsEscuela[mod] && privsEscuela[mod].ver === true) tieneOtrosAccesos = true;
                if(privsEscuela[mod] === true) tieneOtrosAccesos = true;
            }
            return tieneOtrosAccesos;
    ...
    evaluarFuerzaClave: function(clave) {
        let fuerza = 0;
        ...
        let barra = document.getElementById('pi-fuerza-barra');
        let texto = document.getElementById('pi-fuerza-texto');
        
        barra.style.width = fuerza + '%';
        if (fuerza < 50) {
            barra.className = 'progress-bar bg-danger';
            texto.className = 'text-danger d-block mb-3 ps-2';
            texto.innerText = 'Débil (Requiere mayúscula, minúscula, número y símbolo)';
        } else if (fuerza < 75) {
            barra.className = 'progress-bar bg-warning';
            texto.className = 'text-warning d-block mb-3 ps-2';
            texto.innerText = 'Media (Agregue símbolos o números)';
        } else {
            barra.className = 'progress-bar bg-success';
            texto.className = 'text-success d-block mb-3 ps-2';
        ...
        try {
            ...
            if (error || !data || data.rol === 'Invitado' || data.rol === 'Visitante') {
                ...
                return Swal.fire({
                    ...
                    text: `Su cédula no pertenece al personal de ${localStorage.getItem('sigae_escuela_activa') || 'esta institución'}. ¿Desea ingresar como invitado?`,
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#0066FF',
                    confirmButtonText: 'Sí, registrar visita',
            ...
            // Validación dinámica de escuela leyendo el campo perfil_acceso JSON
            let accesoPermitido = false;
            let escuelaActiva = localStorage.getItem('sigae_escuela_activa');

            try {
                let perfil = typeof data.perfil_acceso === 'string' ? JSON.parse(data.perfil_acceso) : data.perfil_acceso;
                if (perfil && perfil.instituciones && Array.isArray(perfil.instituciones)) {
                    accesoPermitido = perfil.instituciones.includes(escuelaActiva);
                }
            } catch (err) {
                console.warn("No se pudo parsear el perfil_acceso o está vacío.", err);
            }

            if (!accesoPermitido && data.rol !== 'Administrador') {
                this.ocultarCarga();
                return Swal.fire('Acceso Denegado', 'Usted está registrado en el sistema, pero no tiene autorización asignada para ingresar a esta institución.', 'error');
            }

            let requiere_config = (data.primer_ingreso === true || !data.clave);
    ...
    iniciarSesion: async function() { 
        ...
        try {
            ...
            if (!accesoPermitido && data.rol !== 'Administrador') {
                this.ocultarCarga();
                return Swal.fire('Acceso Denegado', 'Usted no tiene permisos para esta institución.', 'error');
            }
            ...
            // Chequeo de expiración de contraseña (30 días)
            if (data.fecha_ult_clave) {
                const fechaUltima = new Date(data.fecha_ult_clave);
                const hoy = new Date();
                const diffDias = Math.floor((hoy - fechaUltima) / (1000 * 60 * 60 * 24));
                if (diffDias >= 30) {
                    setTimeout(() => {
                        Swal.fire('Aviso de Seguridad', `Su contraseña tiene ${diffDias} días de antigüedad. Por su seguridad, le recomendamos cambiarla pronto en el módulo de Seguridad.`, 'warning');
                    }, 1000); // Dar 1 seg para que cargue la interfaz inicial
                }
            }
    ...
    iniciarRecuperacion: async function() { 
        ...
        const { data, error } = await window.supabaseDB.from('usuarios').select('preguntas_seguridad, bloqueo_hasta, estado, email').eq('cedula', String(cedula)).maybeSingle();
        this.ocultarCarga();
        ...
        let pregJSON = {};
        try { pregJSON = typeof data.preguntas_seguridad === 'string' ? JSON.parse(data.preguntas_seguridad) : (data.preguntas_seguridad || {}); } catch(e){}
        
        let p1 = pregJSON.pregunta_1, p2 = pregJSON.pregunta_2;
    ...
    procesarRecuperacion: async function() { 
        ...
        if(!correo || !resp || !c1 || !c2) return Swal.fire('Datos Incompletos', 'Complete su correo, respuesta y contraseña.', 'warning'); 
        ...
        const { data, error } = await window.supabaseDB.from('usuarios').select('preguntas_seguridad, email, intentos_fallidos').eq('cedula', String(c)).maybeSingle();
        
        if (error || !data) { this.ocultarCarga(); return Swal.fire('Error', 'Falla de servidor o usuario no encontrado', 'error'); }
        if (String(data.email).trim().toLowerCase() !== correo.toLowerCase()) {
            this.ocultarCarga();
            return Swal.fire('Error', 'El correo ingresado no coincide con el registrado en nuestro sistema.', 'error');
        }

        let pregJSON = {};
        try { pregJSON = typeof data.preguntas_seguridad === 'string' ? JSON.parse(data.preguntas_seguridad) : (data.preguntas_seguridad || {}); } catch(e){}

        let rReal = this.preguntaRecuperacionActiva === 1 ? pregJSON.respuesta_1 : pregJSON.respuesta_2;
        ...
        const { error: updErr } = await window.supabaseDB.from('usuarios').update({ clave: c1, intentos_fallidos: 0, bloqueo_hasta: null, fecha_ult_clave: new Date().toISOString() }).eq('cedula', String(c));
        this.ocultarCarga();
    ...
    evaluarFuerzaClaveRecuperacion: function(clave) {
        let fuerza = 0;
        if (clave.length >= 8) fuerza += 25;
        if (/[A-Z]/.test(clave)) fuerza += 25;
        if (/[a-z]/.test(clave)) fuerza += 25;
        ...
        let barra = document.getElementById('rec-fuerza-barra');
        barra.style.width = fuerza + '%';
        if (fuerza < 50) barra.className = 'progress-bar bg-danger';
        else if (fuerza < 75) barra.className = 'progress-bar bg-warning';
        else barra.className = 'progress-bar bg-success';
    },

    solicitarSoporteAdministrador: async function() {
        const c = document.getElementById('inputCedula').value;
        if(!c) return Swal.fire('Error', 'Debe ingresar su cédula en el inicio.', 'error');
    ...
    accesoInvitado: function() { 
        document.getElementById('paso-cedula').style.display = 'none'; 
        document.getElementById('paso-invitado').style.display = 'block'; 
        let escuela = localStorage.getItem('sigae_escuela_activa') || 'la institución';
        document.getElementById('inv-mensaje-bienvenida').innerHTML = `Visitando: <b>${escuela}</b><br>Verifica y completa tus datos.`;
    },
    ...
    buscarInvitado: async function(cedula) {
        ...
        try {
            ...
            let codigo = localStorage.getItem('sigae_escuela_codigo');
            const nombre = localStorage.getItem('sigae_escuela_activa');
            
            // Inferencia por si el localStorage está en versión vieja
            if (!codigo || codigo === 'null' || codigo === 'undefined') {
                if (nombre && nombre.toLowerCase().includes('santa')) codigo = 'sb';
                else if (nombre && nombre.toLowerCase().includes('libertador')) codigo = 'lb';
                if (codigo && codigo !== 'null') localStorage.setItem('sigae_escuela_codigo', codigo);
            }
            
            const escuelas = [codigo, nombre].filter(Boolean);
            ...
            if (data) {
                document.getElementById('inv-nombres').value = data.nombres || '';
                document.getElementById('inv-apellidos').value = data.apellidos || '';
                document.getElementById('inv-correo').value = data.correo || '';
            ...
            console.error("Excepción en buscarInvitado:", e);
        }
    },

    enviarRegistroInvitado: async function() { 
        const cedulaInv = document.getElementById('inv-cedula').value.trim();
        const nP = document.getElementById('inv-nombres').value.trim(); 
        const nA = document.getElementById('inv-apellidos').value.trim(); 
        ...
        if(!cedulaInv || !nP || !nA || !correoInv || !telefonoInv || !motivoInv) {
        ...
        try {
            ...
            /* BARRERA DE MANTENIMIENTO (Comentado porque conf_sistema no existe)
            const { data: sysConf } = await window.supabaseDB.from('conf_sistema').select('valor_booleano').eq('id_config', 'modo_mantenimiento').maybeSingle();
            if (sysConf && sysConf.valor_booleano === true) {
                this.ocultarCarga();
                return Swal.fire({
            ...
            if (invitadoExistente) {
                const { error: errUpdate } = await window.supabaseDB.from('invitados')
                    .update({ 
                        ...
                        razon_visita: motivoInv,
                        escuela_id: codigo // Nos aseguramos de actualizarlo al código correcto si era viejo
                    })
                    .eq('id_invitado', invitadoExistente.id_invitado);
                ...
                const { error: errInsert } = await window.supabaseDB.from('invitados')
                    .insert([{ 
                        ...
                        telefono: telefonoInv,
                        razon_visita: motivoInv,
                        escuela_id: localStorage.getItem('sigae_escuela_codigo')
                    }]);
    ...
    generarDashboardModulo: function(nombreCategoria) { 
        ...
        <style>
            ...
            .icono-caja { width: 150px; height: 150px; border-radius: 20px; background: white; display: flex; align-items: center; justify-content: center; font-size: 4.5rem; margin-bottom: auto; }
        ...
        // Obtener logo de escuela dinámico
        const codigoEscuelaDash = (this.usuario && this.usuario.id_escuela) ? this.usuario.id_escuela : (localStorage.getItem('sigae_escuela_codigo') || 'sb');
        let logoFileDash = `logo_${codigoEscuelaDash}.png`;
        if (['lb', 'libertador', 'libertadorbolivar'].includes(codigoEscuelaDash.toLowerCase())) logoFileDash = 'logo_lb.png';
        if (['sb', 'santabarbara'].includes(codigoEscuelaDash.toLowerCase())) logoFileDash = 'logo_sb.png';
                ...
                <div class="banner-padre p-4 p-md-5 text-white" style="background: linear-gradient(135deg, ${modulo.color} 0%, rgba(0,0,0,0.4) 150%);">
                    ...
                    <div class="row align-items-center position-relative z-1">
                        ...
                        <div class="col-md-3 text-center text-md-end d-none d-md-block">
                            <img src="assets/img/${logoFileDash}" alt="Logo Escuela" style="max-height: 130px; filter: drop-shadow(0 10px 20px rgba(0,0,0,0.3));">
                        </div>
                    </div>
                </div>
            </div>
    ...
    auditar: function(modulo, accion, detalles = "") {
        ...
        let codEscuela = (this.usuario && this.usuario.id_escuela) ? this.usuario.id_escuela : (localStorage.getItem('sigae_escuela_codigo') || 'sb');
        ...
        try {
            ...
            misNotificaciones.forEach(n => {
                if (!this.alertasActivas.find(a => a.id_notificacion === n.id_notificacion)) {
                    hayNuevas = true;
                    this.mostrarNotificacionChat(n.titulo, n.mensaje, n.created_at);
                }
            });
    ...
    mostrarNotificacionChat: function(titulo, mensaje, timestamp) {
        let contenedor = document.getElementById('contenedor-burbujas-chat');
        if (!contenedor) {
            contenedor = document.createElement('div');
            contenedor.id = 'contenedor-burbujas-chat';
            ...
            contenedor.style.pointerEvents = 'none'; // Para que no bloquee clicks debajo
            document.body.appendChild(contenedor);
        }

        let hora = timestamp ? new Date(timestamp).toLocaleTimeString('es-VE', {hour: '2-digit', minute:'2-digit'}) : 'Ahora';
        let idBurbuja = 'burbuja-' + new Date().getTime() + Math.floor(Math.random() * 100);
        
        let htmlBurbuja = `
            <div id="${idBurbuja}" class="animate__animated animate__slideInRight shadow-lg" style="background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); border-radius: 16px 16px 0px 16px; width: 320px; padding: 14px 18px; border-left: 5px solid #10B981; position: relative; pointer-events: auto; cursor: pointer; transition: all 0.3s ease;">
                <div class="d-flex align-items-center mb-2">
                    <div class="bg-success text-white rounded-circle d-flex align-items-center justify-content-center me-2 shadow-sm" style="width: 32px; height: 32px; flex-shrink: 0;">
                        <i class="bi bi-bell-fill fs-6"></i>
                    </div>
                    <strong class="text-dark mb-0 me-auto" style="font-size: 0.95rem; letter-spacing: -0.3px;">${titulo}</strong>
                    <small class="text-muted fw-bold" style="font-size: 0.7rem;">${hora}</small>
                </div>
                <p class="text-secondary mb-0 ms-1" style="font-size: 0.85rem; line-height: 1.4;">${mensaje}</p>
                <button type="button" class="btn-close position-absolute top-0 end-0 m-2" style="font-size: 0.6rem; opacity: 0.5;" onclick="
                    let el = document.getElementById('${idBurbuja}');
        ...
        div.innerHTML = htmlBurbuja;
        let elemento = div.firstElementChild;
        
        // Al hacer click, cerrar y abrir panel de notificaciones
        elemento.addEventListener('click', () => {
             elemento.classList.replace('animate__slideInRight', 'animate__slideOutRight');
             setTimeout(() => elemento.remove(), 500);
             let panel = document.getElementById('panel-notificaciones');
             if(panel && panel.classList.contains('d-none')) {
                 this.alternarPanelNotificaciones();
             }
        });
    ...
    toggleTemaVisual: function() {
        if (this.temaActivo === 'light') {
            this.temaActivo = 'dark';
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('sigae_tema', 'dark');
            const icono = document.getElementById('icono-tema');
            if (icono) icono.className = 'bi bi-sun-fill fs-4 text-warning hover-efecto';
        } else {
            this.temaActivo = 'light';
            document.documentElement.removeAttribute('data-theme');
            localStorage.setItem('sigae_tema', 'light');
            const icono = document.getElementById('icono-tema');
            if (icono) icono.className = 'bi bi-moon-stars-fill fs-4 text-secondary hover-efecto';
        }
    },

    cargarTemaVisual: function() {
        const temaGuardado = localStorage.getItem('sigae_tema');
        if (temaGuardado === 'dark') {
            this.temaActivo = 'dark';
            document.documentElement.setAttribute('data-theme', 'dark');
            // Como esto se ejecuta muy rápido, aseguramos que el icono cambie si existe
            setTimeout(() => {
                const icono = document.getElementById('icono-tema');
                if (icono) icono.className = 'bi bi-sun-fill fs-4 text-warning hover-efecto';
            }, 100);
    ...
    registrarHuella: async function() {
        if (!window.PublicKeyCredential) {
            return Swal.fire('No soportado', 'Tu dispositivo o navegador actual no soporta tecnología biométrica o Passkeys.', 'warning');
        }
        try {
    ...
    loginBiometrico: async function() {
        let cedula = localStorage.getItem('sigae_tiene_huella');
        if (!cedula) return;
        ...
        try {
            ...
            this.mostrarCarga();
            const { data, error } = await window.supabaseDB.from('usuarios').select('*').eq('cedula', String(cedula)).maybeSingle();
            
            if (error || !data || data.estado !== 'Activo') {
                this.ocultarCarga();
                return Swal.fire('Error', 'Usuario bloqueado o no encontrado.', 'error');
            }
            ...
            if (data.credencial_biometrica !== rawId) {
                this.ocultarCarga();
                return Swal.fire('Acceso Denegado', 'La huella o PIN proporcionado no coincide con el registrado en tu cuenta.', 'error');
            }
            ...
            this.ocultarCarga();
            if (e.name !== "NotAllowedError") {
                Swal.fire('Error', 'No se pudo procesar la verificación biométrica.', 'error');
            }
        }
    }
};
...
window.onload = () => { window.Aplicacion.init(); };